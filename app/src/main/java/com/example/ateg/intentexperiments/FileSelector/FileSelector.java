...
public class FileSelector {
    ...
    public FileSelector(final Context context, final FileOperation operation,
        ...
        this(context, operation, onHandleFileListener, fileFilters, null);
    }

    public FileSelector(final Context context, final FileOperation operation,
                        final OnHandleFileListener onHandleFileListener, final String[] fileFilters, File file) {
        mContext = context;
        mOnHandleFileListener = onHandleFileListener;

        if (file == null ) {
            mCurrentLocation = determineStartingFolder(context);
        } else {
            mCurrentLocation = file.isDirectory() ? file : file.getParentFile();
        }
    ...
    private static File determineStartingFolder(Context context) {
        ...
        if (storageDir.canRead()) {
            ...
            if (sdCard.canRead()) {
                ...
                currentLocation = Environment.getRootDirectory();
            }
        }
